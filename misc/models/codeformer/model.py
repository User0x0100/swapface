...
def load_codeformer(device: torch.device | str = "cuda") -> CodeFormer:
    ...
    weight_path = hf_hub_download(
        repo_id=MODEL_REPOSITORY_ID, filename="codeformer.pth"
    )
...
if __name__ == "__main__":
    ...
    with torch.inference_mode():
        for idx in range(sample_nb):
            image, fn = sample_dir.sample_tensor(
                return_stem=True, device=device
            )  # image: 0.0 ~ 255.0 RGB CHW
