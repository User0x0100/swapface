...
upfirdn2d_fn = torch.ops.upfirdn2d.upfirdn2d
...
def upfirdn2d(
    ...
    """
    if f.ndim == 2:
        x = upfirdn2d_fn(x, f, upx, upy, downx, downy, padx0, padx1, pady0, pady1, flip_filter, gain)
    else:
        x = upfirdn2d_fn(x, f.unsqueeze(0), upx, 1, downx, 1, padx0, padx1, 0, 0, flip_filter, np.sqrt(gain))
        x = upfirdn2d_fn(x, f.unsqueeze(1), 1, upy, 1, downy, 0, 0, pady0, pady1, flip_filter, np.sqrt(gain))
    return x
...
"""
torch.library.register_autograd("upfirdn2d::upfirdn2d", backward, setup_context=setup_context)
