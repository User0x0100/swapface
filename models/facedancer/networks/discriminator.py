import torch
from torch import nn, Tensor
from .generator import get_act, BASE_CH, MAX_CH


class ResidualDownBlock(nn.Module):
    def __init__(self, in_ch: int, out_ch: int, resample=True) -> None:
        super().__init__()

        self.shortcut = nn.Sequential(
            nn.Conv2d(in_ch, out_ch, kernel_size=1, stride=1, padding=0),
            *([nn.AvgPool2d(kernel_size=2, stride=2)] if resample else []),
        )
        self.residual = nn.Sequential(
            nn.InstanceNorm2d(in_ch, affine=True),
            get_act(),
            nn.Conv2d(in_ch, out_ch, kernel_size=3, stride=1, padding=1),
            *([nn.AvgPool2d(kernel_size=2, stride=2)] if resample else []),
            nn.InstanceNorm2d(out_ch, affine=True),
            get_act(),
            nn.Conv2d(out_ch, out_ch, kernel_size=3, stride=1, padding=1),
        )

    def forward(self, x: Tensor) -> Tensor:
        return self.residual(x) + self.shortcut(x)


class Discriminator(nn.Module):
    def __init__(self, input_res: int = 256, bottleneck_res: int = 8) -> None:
        super().__init__()

        num_down = int(torch.log2(torch.tensor(input_res // bottleneck_res)))

        self.conv_first = nn.Conv2d(3, BASE_CH, kernel_size=3, stride=1, padding=1)
        self.blocks = nn.Sequential()

        down_in_ch = BASE_CH
        for _ in range(num_down):
            down_out_ch = min(down_in_ch * 2, MAX_CH)
            self.blocks.append(ResidualDownBlock(down_in_ch, down_out_ch, resample=True))
            down_in_ch = down_out_ch

        self.final_conv = nn.Sequential(
            nn.Conv2d(down_in_ch, down_in_ch, kernel_size=bottleneck_res, stride=1, padding=0),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(down_in_ch, 1, kernel_size=1, stride=1, padding=0),
        )

    def forward(self, x: Tensor) -> Tensor:

        x = self.conv_first(x)
        x = self.blocks(x)
        x = self.final_conv(x)
        x = x.view(x.size(0), -1)

        return x


if __name__ == "__main__":
    size = 512
    model = Discriminator(size)
    x = torch.randn(2, 3, size, size)
    out = model(x)
    print(out.shape)
