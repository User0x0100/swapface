...
class Encoder_noBNIN(nn.Module):
    def __init__(self, id_feature_dim):
        ...
        self.fusion_module = nn.ModuleDict({f"fusion_layer_{i}": Cross_Adaptive_Identity_Injdection_Block(1024, 2048, 512) for i in range(6)})
...
class Swapper(nn.Module):  # class Swapper_Enlarge(pl.LightningModule):
    ...
    def forward(self, target, id_feat, get_latent=False):
        output = self.E(target, id_feat)
        if get_latent:
